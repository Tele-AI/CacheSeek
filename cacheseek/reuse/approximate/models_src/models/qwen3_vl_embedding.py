...
# Adapted from the Qwen3-VL-Embedding reference implementation
# (Qwen team, Alibaba Cloud; https://huggingface.co/Qwen/Qwen3-VL-Embedding-2B)
...
def sample_frames(
    ...
    """Uniformly sample the final number of frames across the full video."""
    if not frames:
        raise ValueError("sample_frames requires at least one frame")
    duration = len(frames)
    target_segments = max(1, min(int(num_segments), int(max_segments)))
    frame_id_array = np.linspace(0, duration - 1, target_segments, dtype=int)
    frame_id_list = frame_id_array.tolist()

    # Create a list of sampled frames
    sampled_frames = []
    for frame_idx in frame_id_list:
        if frame_idx < 0 or frame_idx >= duration:
            raise IndexError(f"sample_frames generated out-of-range index={frame_idx} duration={duration}")
...
class Qwen3VLEmbedder:
    def __init__(
        ...
        **kwargs,
        ...
        self.model = Qwen3VLForEmbedding.from_pretrained(model_name_or_path, trust_remote_code=True, **kwargs).to(
            device
        )
    ...
    def forward(self, inputs: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        return {"last_hidden_state": outputs.last_hidden_state, "attention_mask": inputs.get("attention_mask")}
    ...
    def _preprocess_inputs(self, conversations: List[List[Dict]]) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            logger.exception(
                "Qwen3VLEmbedder failed to process multimodal inputs conversation_count=%s err=%s",
                len(conversations),
                exc,
            )
            raise RuntimeError(
                f"Qwen3VLEmbedder failed to process multimodal inputs conversation_count={len(conversations)} err={exc}"
            ) from exc

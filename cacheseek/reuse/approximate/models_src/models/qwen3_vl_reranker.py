...
class Qwen3VLReranker:
    def __init__(
        ...
        **kwargs,
        ...
        lm = Qwen3VLForConditionalGeneration.from_pretrained(model_name_or_path, trust_remote_code=True, **kwargs).to(
            self.device
        )

        self.model = lm.model
        self.processor = AutoProcessor.from_pretrained(model_name_or_path, trust_remote_code=True, padding_side="left")
        self.model.eval()
    ...
    def _move_inputs_to_device(self, tokenized_inputs: Dict) -> Dict:
        if hasattr(tokenized_inputs, "to"):
            return tokenized_inputs.to(self.model.device)
        return {
            key: value.to(self.model.device) if hasattr(value, "to") else value
            for key, value in tokenized_inputs.items()
        }

    def _normalize_score_list(self, scores: Union[List[float], tuple, float]) -> List[float]:
    ...
    def _score_pairs_batched(self, batch_pairs: List[Dict]) -> List[float]:
        tokenized_inputs = self.tokenize(batch_pairs)
        tokenized_inputs = self._move_inputs_to_device(tokenized_inputs)
        scores = self._normalize_score_list(self.compute_scores(tokenized_inputs))
        if len(scores) != len(batch_pairs):
            raise ValueError(f"score size mismatch: expected {len(batch_pairs)} got {len(scores)}")
        return scores

    def _score_pairs_serial(self, batch_pairs: List[Dict]) -> List[float]:
        final_scores: List[float] = []
        for pair in batch_pairs:
            final_scores.extend(self._score_pairs_batched([pair]))
        return final_scores
    ...
    def tokenize(self, pairs: List[Dict], **kwargs) -> Dict:
        ...
        try:
            ...
            logger.exception(
                "Qwen3VLReranker failed to process multimodal pairs pair_count=%s err=%s",
                len(pairs),
                exc,
            )
            raise RuntimeError(
                f"Qwen3VLReranker failed to process multimodal pairs pair_count={len(pairs)} err={exc}"
            ) from exc
        ...
        for i, ele in enumerate(inputs["input_ids"]):
            inputs["input_ids"][i] = (
                self.truncate_tokens_optimized(
                    inputs["input_ids"][i][:-5], max_length, self.processor.tokenizer.all_special_ids
                )
                + inputs["input_ids"][i][-5:]
            )

        # Apply padding. padding=True pads to the longest sequence in the batch;
        # truncation to max_length already happened above (truncate_tokens_optimized),
        # so a max_length kwarg here would be silently ignored by .pad() — omit it
        # to avoid the "max_length is ignored when padding=True" warning. (Do NOT
        # switch to padding='max_length': that pads every batch to MAX_LENGTH.)
        temp_inputs = self.processor.tokenizer.pad(
            {"input_ids": inputs["input_ids"]}, padding=True, return_tensors="pt"
        )
    ...
    def format_mm_instruction(
        ...
        contents = []
        contents.append({"type": "text", "text": "<Instruct>: " + (instruct or self.default_instruction)})

        # Format query content
        query_content = self.format_mm_content(
            query_text, query_image, query_video, prefix="<Query>:", fps=fps, max_frames=max_frames
        )
    ...
    def process(
        ...
        for start in range(0, len(pairs), batch_size):
            ...
            try:
                final_scores.extend(self._score_pairs_batched(batch_pairs))
            except Exception as exc:
                logger.exception(
                    "Qwen3VLReranker batch scoring failed start=%s size=%s err=%s",
                    start,
                    len(batch_pairs),
                    exc,
                )
                raise RuntimeError(
                    "Qwen3VLReranker batch scoring failed "
                    f"start={start} size={len(batch_pairs)} "

...
<p align="center"><em>Wan2.2-14B T2V，2×80GB Hopper。每列上面是冷启动完整去噪的原片，下面是命中缓存的后续请求 —— 复用上面那条作 donor（早期去噪 latent、跳过前 K 步），同场景重渲并带镜头 / 风格变化。具身 / 世界模型场景：街道机器人、厨房端盘、桌面抓取、插花。</em></p>

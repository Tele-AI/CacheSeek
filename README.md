...
<p align="center"><em>Wan2.2-14B T2V on 2×80GB Hopper. In each column the top clip is generated cold (full denoise); the bottom is a follow-up request served from cache — it reused the top as donor (early-denoise latents, skipping the first K steps) and re-rendered the same scene with a camera / style shift. Embodied-AI / world-model scenes: sidewalk robot, kitchen tray, table pick, flower arranging.</em></p>
